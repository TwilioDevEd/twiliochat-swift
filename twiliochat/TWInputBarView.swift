...
//  twiliochat
//
//  Created by HackMini on 13.12.2019.
...
protocol TWInputBarDelegate {
	func inputBarDidSelectImage(_ inputBar: InputBarAccessoryView, image: UIImage)
}

class TWInputBarView: InputBarAccessoryView {
	
	open var inputBarDelegate: TWInputBarDelegate?
...
extension TWInputBarView: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
	...
	func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
		...
		picker.dismiss(animated: true, completion: {
			if let pickedImage = info[convertFromUIImagePickerControllerInfoKey(UIImagePickerController.InfoKey.originalImage)] as? UIImage {
				...
				self.inputBarDelegate?.inputBarDidSelectImage(self, image: pickedImage)
			}
		})
	}
}
